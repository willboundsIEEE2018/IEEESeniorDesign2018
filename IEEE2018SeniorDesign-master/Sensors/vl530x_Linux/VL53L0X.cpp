...
void VL53L0X::writeRegister(uint8_t reg, uint8_t value) {
  ...
  write(this->i2cFileDescriptor, data_write, 2);
}
...
void VL53L0X::writeRegister16Bit(uint8_t reg, uint16_t value) {
  ...
  write(this->i2cFileDescriptor, data_write, 3);
}
...
void VL53L0X::writeRegister32Bit(uint8_t reg, uint32_t value) {
  ...
  data_write[0] = (reg) & 0xFF;
	data_write[1] = (value>>24) & 0xFF;
	data_write[2] = (value>>16) & 0xFF;  
  data_write[3] = (value>>8) & 0xFF;  
  data_write[4] = value & 0xFF;
  write(this->i2cFileDescriptor, data_write, 5);
}
...
void VL53L0X::writeRegisterMultiple(uint8_t reg, const uint8_t* data, uint8_t count) {
  ...
  memcpy(buf+1, data, count);
    if (write(fd, buf, count+1) != count+1) {
      printf("Failed to write to the i2c bus.\n");
      free(buf);
      return VL53L0X_ERROR_CONTROL_INTERFACE;
    }
  free(buf);
  write(this->i2cFileDescriptor, data_write, count);
}

uint8_t VL53L0X::readRegister(uint8_t reg) {
  uint8_t data_read;
  write(this->i2cFileDescriptor, reg, 1);
  read(this->i2cFileDescriptor, data_read, 1);
  return data_read;
}
...
uint16_t VL53L0X::readRegister16Bit(uint8_t reg) {
  uint16_t data_read;
  write(this->i2cFileDescriptor, reg, 1);
  read(this->i2cFileDescriptor, data_read, 2);
  return data_read;
}
...
uint32_t VL53L0X::readRegister32Bit(uint8_t reg) {
  uint32_t data_read;
  write(this->i2cFileDescriptor, reg, 1);
  read(this->i2cFileDescriptor, data_read, 3);
  return data_read;
}

void VL53L0X::readRegisterMultiple(uint8_t reg, uint8_t* destination, uint8_t count) {
    if (write(this->i2cFileDescriptor, &reg, 1) != 1) {
        printf("Failed to write to the i2c bus.\n");
        return VL53L0X_ERROR_CONTROL_INTERFACE;
    }

    if (read(this->i2cFileDescriptor, destination, count) != count) {
