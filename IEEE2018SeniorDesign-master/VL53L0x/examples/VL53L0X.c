...
VL53L0X_Error rangingTest(VL53L0X_Dev_t *pMyDevice)
{
    ...
    if(Status == VL53L0X_ERROR_NONE)
    {
// no need to do this when we use VL53L0X_PerformSingleRangingMeasurement printf ("Call of VL53L0X_SetDeviceMode\n");
   ...
   while(1) {
	Status = VL53L0X_PerformSingleRangingMeasurement(pMyDevice,&RangingMeasurementData);      
         if (RangingMeasurementData.RangeStatus == 0)
            printf("Measured distance: %4i\n", RangingMeasurementData.RangeMilliMeter);
         else
            printf("Measured distance: XXXX\n");
	}
